	...
	class EasyCode {
		...
		function getreplacestr($codeid){
			...
			return $replacestr = '<div class="ecodebox" id="codebox_'."$codeid".'" ondblclick="EasyCodeClass.ChageListStyle(this.id,event)">
						...
						<a id="textareaboxcode_'.$codeid.'" class="copycode" onclick="EasyCodeClass.ShowTextAreaBox(event,this.id);" href="javascript:;">['.translate('EDIT','easycode').']</a>
...
<div id="EasyCodeTextAreaBox" class="ec_textareabox">
	<div class="close_b"><a href="javascript:;" onclick="EasyCodeClass.CloseTextAreaBox();"></a></div>
...
<script type="text/javascript">
	...
	function newEasyCodeClass(codeid){
		...
		this.CopyCode = function(id){
			...
			var text = this.HtmlTo(TagName[0].innerHTML);
		...
		this.ShowTextAreaBox=function(event,id){
			id = id.replace(/textareaboxcode_/,'codebox_');
			textareaobj = document.getElementById('EasyCodeTextAreaBox');
			codeboxobj = document.getElementById(id);
			var TagName = document.getElementById(id).getElementsByTagName('ol');
			var text = this.HtmlTo(TagName[0].innerHTML);

			textareaobj.style.display = 'block';
			//textareaobj.style.top = offset+'px';
			//textareaobj.style.left = event.clientX+'px';
			textareaobj.getElementsByTagName('textarea')[0].innerHTML = text;
		}

		this.CloseTextAreaBox = function(){
			textareaobj = document.getElementById('EasyCodeTextAreaBox');
