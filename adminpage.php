...
if(isset($_POST['submit'])){
	...
	$option['use_user_style'] = (bool)$_POST['use_user_style'];
	$option['default_style'] = in_array($_POST['default_style'],$slist,true)?$_POST['default_style']:'Gray';
...
$hiddenradio = array($option['hidden']=>' checked="checked"');
$userstyleradio = array($option['use_user_style']=>' checked="checked"');
...
<tr valign="top">
	...
	<td>
		<label><input type="radio" name="hidden" value="1" <?php echo $hiddenradio[1]; ?>><?php _e('Collapsing code','easycode'); ?></label><br />
		<label><input type="radio" name="hidden" value="0" <?php echo $hiddenradio[0]; ?>><?php _e('Do not collapse, enable collapsing','easycode'); ?></label><br />
	</td>
</tr>
<tr valign="top">
...
<tr valign="top">
<td><?php _e('Formatted Tag','easycode'); ?></td>
<td>
...
<div id="op-appearance" class="" style="display:none;">
	<p id="use_user_style">
		<label><input type="radio" name="use_user_style" value="0" onclick="ecAdmin.changeStyleRadio(0);" <?php echo $userstyleradio[0];?>><?php _e('Use system color schemes','easycode');?>&nbsp;&nbsp;</label>
		<select name="default_style" id="default_style">
		<?php if (!in_array($option['default_style'],$slist,TRUE)) $option['default_style']='Gray';
		foreach($slist as $sname){
			$che = ($option['default_style']===$sname)?' selected="selected"':'';
			echo "\t\t\t<option value=\"$sname\" $che>$sname</option>";
		}?>
		</select>
		<br /><br />
		<label><input type="radio" name="use_user_style" value="1" onclick="ecAdmin.changeStyleRadio(1);" <?php echo $userstyleradio[1];?>><?php _e('Use custom color schemes','easycode');?></label>
	</p>
	...
	<table id="inputcolor">
		...
		<tr>
			<td></td>
			<td>
			<label><input type="checkbox" name="nowrap" value="1" <?php echo $option['nowrap']?'checked="checked"':''; ?>><?php _e('No Wrap','easycode'); ?></label><br />
			</td>
	</table>
</div>
